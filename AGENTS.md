...
- **Text**: `<Text variant="h1|h2|h3|body|caption|code|watermark">` only. No raw `<h1>`, `<p>`, etc.
...
- **Box `h`**: Must be ≥ text line box. Font metrics: h1=72px(86), h2=54px(70), h3=42px(59), body=30px(48), caption=22px(33). For bordered Cardbox, subtract 2×border px from effective h.
...
| Path | Purpose |
|---|---|
| `@/components/text` | `<Text variant="h1\|h2\|h3\|body\|caption\|code\|watermark">` |
| `@/components/anim` | `<Anim type="fade-in\|slide-*\|scale-in" delay={ms}>` |
| `@/components/cardbox` | Card container (default/elevated/bordered) |
| `@/components/markdown` | Markdown renderer (LaTeX, GFM tables) |
...
- Detection: top-level siblings only (no parent-child false positives). 1px tolerance for touching edges.
